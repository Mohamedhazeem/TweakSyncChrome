 ...
 * the side-panel tree through the single composition root. Every component thus
 * reaches the browser runtime only through the injected ports, keeping the UI
 * free of direct extension calls.
 */
export function ExtensionProvider({ children }: { children: ReactNode }) {
  const [value] = useState<ExtensionContextValue>(() => {
    const composition = createExtensionComposition();
    return { browser: composition.browser, syncService: composition.syncService };
  });
