...
function AddStyleProperty({
  ...
  const handleSelect = (newValue: string) => {
    ...
    setStyles((prevStyles) => {
      const newStyleProperty = GLOBAL_STYLES.find(
        (style) => style.name === newValue
      );
      ...
      if (newStyleProperty) {
        ...
        if (selector === "inline") {
          updatedStyles.inline[newStyleProperty.name] = "";
        } else if (updatedStyles.external.classes[selector]) {
          updatedStyles.external.classes[selector][newStyleProperty.name] = "";
        } else if (updatedStyles.external.ids[selector]) {
          updatedStyles.external.ids[selector][newStyleProperty.name] = "";
        } else if (updatedStyles.external.tags[selector]) {
          updatedStyles.external.tags[selector][newStyleProperty.name] = "";
        } else if (updatedStyles.external.attribute[selector]) {
          updatedStyles.external.attribute[selector][newStyleProperty.name] =
            "";
        } else if (updatedStyles.external.descendant[selector]) {
          updatedStyles.external.descendant[selector][newStyleProperty.name] =
            "";
        } else if (updatedStyles.external.pseudoElementStyles[selector]) {
          updatedStyles.external.pseudoElementStyles[selector][
            newStyleProperty.name
          ] = "";
        } else if (updatedStyles.external.pseudoClassStyles[selector]) {
          updatedStyles.external.pseudoClassStyles[selector][
            newStyleProperty.name
          ] = "";
        } else if (updatedStyles.external.atRules[selector]) {
          for (const subSelector in updatedStyles.external.atRules[selector]) {
            updatedStyles.external.atRules[selector][subSelector][
              newStyleProperty.name
            ] = "";
          }
        } else {
        ...
        addStyleProperty(selector, newStyleProperty.name);
        ...
        <Command>
          ...
          <CommandList>
            ...
            {GLOBAL_STYLES.length === 0 && (
              <CommandEmpty>No style properties found.</CommandEmpty>
            )}
          </CommandList>
        </Command>
      </PopoverContent>
